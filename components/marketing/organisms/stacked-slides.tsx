...
export type StackedSlide = {
  ...
  tone: string;
  /** Laid centred above the headline, such as a `MetricCard`. */
  widget?: ReactNode;
  headline: string;
  body: string;
  image: {
 ...
 * centred in the space between the pill and the headline, whose accents
 * take the same colour.
 */
const classes = {
  root: "relative w-full [--strip:64px]",
  slide:
    "sticky top-[calc(var(--i)*var(--strip))] h-[calc(100dvh-var(--i)*var(--strip))] min-h-[480px] w-full overflow-hidden bg-primary-950 shadow-[0_-24px_48px_-16px_rgb(0_0_0/0.6)]",
  ...
  // any picture.
  pill: "-translate-x-1/2 absolute top-4 left-1/2 m-0 inline-flex items-center whitespace-nowrap rounded-full border border-[color-mix(in_oklab,var(--tone)_60%,transparent)] bg-[color-mix(in_oklab,var(--tone)_14%,rgb(15_18_25/0.72))] px-4 py-2 font-sans font-semibold text-[13px] text-[var(--tone)] leading-none backdrop-blur-md",
  // Three rows: the words in the middle, with equal rows above and below so
  // they stay centred on the slide, and the widget centred in the row above,
  // between the pill and the headline. The row above is never shorter than
  // the widget and its breathing room, so on a short window the words give
  // way downward rather than the widget crowding them.
  message:
    "absolute inset-0 grid grid-rows-[minmax(min-content,1fr)_auto_1fr] justify-items-center px-6 text-center",
  // Padded to keep 32px either side of the widget: from the bottom of the
  // pill (16px from the top plus its 31px height) above, and from the
  // headline below. Equal room each side, so it still centres between them.
  // The figure accent is pointed at the slide colour, so a figure drawing in
  // the widget, such as the lab panel, draws its highlights in the colour of
  // the pill above it.
  widgetRow:
    "flex items-center justify-center pt-[79px] pb-8 [--color-figure-accent:var(--tone)]",
